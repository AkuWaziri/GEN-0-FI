...
import { LayoutDashboard, History, Settings, ArrowLeftRight, Trophy, Send, Sparkles } from 'lucide-react';
...
export const MobileNav: React.FC<MobileNavProps> = ({ activeTab, onSelectTab }) => {
  ...
  return (
    <nav id="mobile-bottom-nav" role="tablist" aria-label="Mobile Navigation" className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#08090b]/95 backdrop-blur-lg border-t border-zinc-900 px-1 py-1.5 flex items-center justify-around select-none shadow-lg pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))]">
      {navItems.map((item) => {
